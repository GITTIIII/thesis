...
export async function GET(request: NextRequest) {
	...
	//   );
	// }
	const qualificationExamCommitteeForm = await db.qualificationExamCommitteeForm.findUnique({
		where: {
	...
	const examDay = new Date(qualificationExamCommitteeForm.examDay).toLocaleDateString("th").split("/");
	const data = {
    ...
    headSchoolSchool: qualificationExamCommitteeForm.headSchool?.school?.schoolNameTH,
	};
	try {
		const file = await genDocx("src/lib/formToDocx/docTemplate/FM-ENG-GRD-02.docx", data);
