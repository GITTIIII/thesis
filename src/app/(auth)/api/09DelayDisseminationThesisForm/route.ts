...
export async function POST(req: Request) {
	try {
		...
		const { date, studentID, headCommitteeName, thesisNameTH, thesisNameEN, startDate, endDate, studentSignUrl, publishmentName } =
			body;
...
export async function PATCH(req: Request) {
	try {
		...
		const body = await req.json();
		const { id, instituteSignUrl, instituteID, approve, dayApprove, timeApprove, disapproveComment } = body;
