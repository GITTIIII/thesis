...
export default function FormDashborad() {
	...
	const { data: form01Data = [] } = useSWR<IComprehensiveExamCommitteeForm[]>(
		process.env.NEXT_PUBLIC_URL + "/api/01ComprehensiveExamCommitteeForm",
		fetcher
	);
	const { data: form02Data = [] } = useSWR<IQualificationExamCommitteeForm[]>(
		process.env.NEXT_PUBLIC_URL + "/api/02QualificationExamCommitteeForm",
		fetcher
	);
	const { data: form03Data = [] } = useSWR<IOutlineCommitteeForm[]>(
		process.env.NEXT_PUBLIC_URL + "/api/03ThesisOutlineCommitteeForm",
		fetcher
	);
	const { data: form04Data = [] } = useSWR<IExamCommitteeForm[]>(process.env.NEXT_PUBLIC_URL + "/api/04ThesisExamCommitteeForm", fetcher);
	const { data: form05Data = [] } = useSWR<IOutlineForm[]>(process.env.NEXT_PUBLIC_URL + "/api/05OutlineForm", fetcher);
	const { data: form06Data = [] } = useSWR<IThesisProgressForm[]>(process.env.NEXT_PUBLIC_URL + "/api/06ThesisProgressForm", fetcher);
	const { data: form07Data = [] } = useSWR<IThesisExamAppointmentForm[]>(
		process.env.NEXT_PUBLIC_URL + "/api/07ThesisExamAppointmentForm",
		fetcher
	);
	const { data: form08Data = [] } = useSWR<IThesisExamAssessmentForm[]>(
		process.env.NEXT_PUBLIC_URL + "/api/08ThesisExamAssessmentForm",
		fetcher
	);
