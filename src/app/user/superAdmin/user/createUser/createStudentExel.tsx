...
import { GoAlertFill, GoChevronDown, GoChevronRight, GoTrash, GoX, GoDownload } from "react-icons/go";
...
export default function CreateStudentExel() {
	...
	const ListTemPlate = ({ index, value }: { index: number; value: { key: string; value: string } }) => {
		const userPropertyIndex = userProperty.findIndex((userP) => userP.key === value.value);
		const userPropertyValue = userPropertyIndex !== -1 ? userProperty[userPropertyIndex].value : "เลือกค่าคีย์";
		return (
	...
	const AddList = () => {
		return (
			<div
				...
				onClick={() => setList((prevList) => [...prevList, { key: "เลือกคอลัมน์", value: "เลือกค่าคีย์" }])}
			>
				<GoX
					size={30}
					style={{
	...
	const onSubmit = async () => {
		setDisabled(true);
		if (list.some((item) => item.key === "เลือกคอลัมน์" || item.value === "เลือกค่าคีย์")) {
			toast({
				variant: "destructive",
				description: (
					<div className=" flex items-center">
						{<GoAlertFill className="mr-2 " size={30} />}
						<p>โปรดเลือกคอลัมและกรอกค่าคีย์ให้ครบทุกข้อ ถ้ามีรายการใดที่ไม่ได้เลือก กรุณาลบข้อมูลนั้น</p>
					</div>
				),
			});
			setDisabled(false);
		...
		fetch(process.env.NEXT_PUBLIC_URL + `/api/user/importExcel`, {
			method: "POST",
			body: data,
		})
	...
	return (
		<div className=" space-y-16">
			<ResizablePanelGroup direction="horizontal">
					...
					<div className="mt-8 w-fit flex flex-col gap-4">
						<div className="flex flex-col gap-3">
							...
							<Button onClick={onSubmit} disabled={disabled}>
								{disabled && <AiOutlineLoading3Quarters className="mr-2 h-4 w-4 animate-spin" />}
								ยืนยัน
							</Button>
						</div>
					</div>
				...
				<ResizablePanel className="text-sm p-4 overflow-scroll" style={{ overflow: "scroll" }}>
										...
										<TableHead key={key}>
											<div className="flex flex-col text-center items-center">
												...
												<p>
													{
														userProperty.find(
															(prop) => prop.key === list.find((item) => item.key === data.key)?.value
														)?.value
													}
												</p>
											</div>
							...
							<TableRow>
								<TableCell colSpan={4} className="text-right text-red-800 ">
									<div className=" space-x-6 flex justify-end ">
										...
										<p className=" grid place-items-center">{`บันทึกไม่สำเร็จ ${countStatus(result)[1]}`}</p>
										{buffer && (
											<Button
												variant="outline"
												onClick={() => {
