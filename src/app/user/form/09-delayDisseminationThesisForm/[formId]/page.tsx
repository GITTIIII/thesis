...
import DelayDisseminatipnFormRead from "@/components/form/09-delayDisseminationThesisForm/09-delayDisseminationThesisFormRead";
import { get09FormById } from "@/app/action/getFormById";
import { currentUser } from "@/app/action/current-user";

export default async function DelayDisseminationThesisFormReadPage({ params }: { params: { formId: number } }) {
			...
			<div className="w-full h-max bg-transparent py-12 px-2 lg:px-28">
				...
				<div className="h-full w-full flex items-center bg-[#EEEEEE] p-2 md:p-8 rounded-md">
					<div className="w-full h-full">
						<DelayDisseminatipnFormRead formData={formData} user={user} />
					</div>
				</div>
			</div>
		</>
