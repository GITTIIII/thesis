...
const ThesisProgressFormCreate = ({
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    setLoading(true);
    const checkSum = checkPlannedWorkSum(
      last06Form?.processPlan ? last06Form?.processPlan : approvedForm.processPlan
    );
    if (!checkSum[0]) {
      toast({
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="w-full h-full bg-white p-4">
        <div className="flex flex-col justify-center md:flex-row">
          ...
          <div className="w-full">
            ...
            />
            <InputForm
              value={`${user?.school?.schoolNameTH}`}
              label="สาขาวิชา / School"
            />
            <InputForm
              value={`${user?.program?.programNameTH}`}
            ...
            <div className="w-full sm:w-max mx-auto p-5 flex flex-col item-center justify-center border-2 rounded-lg mb-5 border-[#eeee]">
              ...
              <InputForm
                value={`${approvedForm?.thesisNameTH}`}
          ...
          <div className="w-full">
            ...
            <div className="w-full flex flex-col items-center mt-6 mb-6 justify-center">
              ...
              <SignatureDialog
                signUrl={
                  user?.signatureUrl && user.role === "STUDENT" ? user?.signatureUrl : ""
                }
                disable={true}
              />
              <Label className="mt-2">{`วันที่ ${
                form.getValues().date
                  ? new Date(form.getValues().date).toLocaleDateString("th")
                  : "__________"
              }`}</Label>
            </div>
          </div>
        ...
        <div className="w-full h-full bg-white p-4 lg:p-12 rounded-lg mt-4">
          ...
          <div className="w-full flex flex-col sm:flex-row justify-center items-center mb-2 ">
            ...
            <Input
              disabled
              className="w-max mx-4 my-2 sm:my-0"
              value={`${approvedForm?.thesisStartMonth}`}
            />
            <Label className="mx-4 font-bold"> ปี พ.ศ.</Label>
            <Input
              disabled
              className="w-max my-2 sm:my-0"
              value={`${approvedForm?.thesisStartYear}`}
            />
          </div>
          <div className="w-full h-max overflow-auto flex justify-center">
            {approvedForm && (
              <ThesisProcessPlan
                ...
                canEdit={true}
                processPlans={
                  last06Form?.processPlan
                    ? last06Form?.processPlan
                    : approvedForm.processPlan
                }
