...
const DelayDisseminationThesisFormRead = ({ user, formData }: { user: IUser; formData: IDelayThesisForm }) => {
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		...
		if (values.instituteSignUrl == "" && user.position === "HEAD_OF_INSTITUTE") {
			toast({
				title: "Error",
	...
	useEffect(() => {
		reset({
			...
			id: formData.id,
			instituteID:
				formData?.instituteID && formData.instituteID > 0
					? formData.instituteID
					: user.position === "HEAD_OF_INSTITUTE"
					? user.id
					: 0,
		});
					...
					<div className="w-full ">
							...
							<div className="w-3/4 flex flex-col item-center justify-center border-2 rounded-lg py-5 my-5 border-[#eeee] ">
								<div>
									{(user.role == "STUDENT" ||
										...
										(user.position == "HEAD_OF_INSTITUTE" && formData)) && (
						...
						<div className="flex item-center justify-center ">
							{(user.role == "STUDENT" ||
								...
								<div className="w-3/4 flex flex-col item-center justify-center border-2 rounded-lg py-5 my-5 border-[#eeee] ">
									<div className="text-center mb-2">
										ผลการพิจารณาของคณะกรรมการประจำสำนักวิชา / <br /> Institute Committee Decision
									</div>
														...
														<FormControl>
															...
															>
																<FormItem className="flex items-center space-x-3 space-y-0">
																	...
																	<FormLabel className="font-normal">อนุมัติ / approve</FormLabel>
																</FormItem>
																<FormItem className="flex items-center space-x-3 space-y-0">
																	<FormControl>
																		<RadioGroupItem value="disApprove" />
																	</FormControl>
																	<FormLabel className="font-normal">ไม่อนุมัติ / disApprove</FormLabel>
																</FormItem>
															</RadioGroup>
														</FormControl>
														<FormMessage />
											...
											<div className="flex justify-center item-center">
												<SignatureDialog
													...
													signUrl={
														formData?.instituteSignUrl
															? formData?.instituteSignUrl
															: form.getValues("instituteSignUrl")
													}
