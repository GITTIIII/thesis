  ...
  const { data: formData } = useSWR<IExamForm>(formId ? `/api/get08FormById/${formId}` : "", fetcher);
	...
	const form = useForm({
		...
		defaultValues: {
			...
			newNameEN:"",
			reviseTitle: false,
      committeeSignUrl:formData && formData.committeeSignUrl? formData.committeeSignUrl : [{ name: "",signUrl:"" }, { name: "" ,signUrl:""}, { name: "",signUrl:"" }, { name: "",signUrl:"" }, { name: "" ,signUrl:""}],
      presentationComment:"",
      explanationComment:"",
          ...
          <div className="text-center">
            ...
            <div>
              <div className="w-full flex flex-row flex-wrap item-center justify-center">
                          ...
                          <SignatureDialog
                            signUrl={
                              formData?.committeeSignUrl?.[index]?.signUrl ? formData?.committeeSignUrl?.[index]?.signUrl : form.getValues(`committeeSignUrl.${index}.signUrl`)
                          }
