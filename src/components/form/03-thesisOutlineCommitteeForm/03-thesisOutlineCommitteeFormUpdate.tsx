							...
							<div className="m-auto w-[400px] mb-6">
								<RadioGroup disabled className="mt-2 flex flex-col justify-center ">
									...
									<div className="flex items-center space-x-3 space-y-0">
										...
										<Label className="ml-2 font-normal leading-5">
											นักศึกษาบัณฑิตศึกษาเป็นผู้ได้รับทุน OROG / <br />
											Graduate students are OROG scholarship recipients.
										</Label>
									</div>
								</RadioGroup>
							</div>
					...
					<div className="w-full flex flex-col item-center justify-center md:flex-row rounded-lg py-5 my-5">
						{(formData?.advisorSignUrl || user.role == "SUPER_ADMIN" || (user.role == "ADMIN" && user.position !== "NONE")) && (
							<div className="w-full sm:1/3 p-0 flex flex-col items-center mb-6">
								...
								/>
								<Label className="mb-6">{`${formData?.student?.advisor?.prefix?.prefixTH}${formData?.student?.advisor?.firstNameTH} ${formData?.student?.advisor?.lastNameTH}`}</Label>
							</div>
						)}
