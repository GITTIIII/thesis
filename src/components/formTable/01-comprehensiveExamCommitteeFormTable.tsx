  ...
  if (formData.headSchoolID) {
    try {
      const response = await fetch(
        `/api/01ComprehensiveExamCommitteeForm/download?id=${formData.id}`
      );
      if (response.ok) {
                    ...
                    <Link
                      href={
                        formData.headSchoolID || userData?.role.toString() == "STUDENT"
                          ? `/user/form/comprehensiveExamCommitteeForm/${formData.id}`
                          : `/user/form/comprehensiveExamCommitteeForm/update/${formData.id}`
                      }
                    >
