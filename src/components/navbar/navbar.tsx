...
const Navbar: React.FC<Props> = ({ menu, notification = false }) => {
  ...
  return (
    <>
      <nav className=" flex px-6 h-16 bg-white justify-between items-center shadow">
        ...
        <ul className=" items-center gap-6 flex">
          ...
          <li>
            <Link href="">
              <div className="h-12 w-12 rounded-full border-2 border-[#6E6D70] content-center items-center overflow-hidden  hidden md:flex animate-fade-up animate-once">
                <Image src={profile?.Avatar!} width={48} height={48} alt="" />
              </div>
            </Link>
          </li>
  ...
  return (
    ...
    >
      ...
      {isComponentVisible && (
        <div
          ref={ref}
          className="origin-top-right absolute right-0 mt-2 w-[500px] p-2 rounded-lg shadow-lg bg-white ring-1 ring-black ring-opacity-5 z-10"
        >
          <ul>
